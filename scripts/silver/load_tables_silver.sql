-- PROCEDURE: silver.load_tables()

-- DROP PROCEDURE IF EXISTS silver.load_tables();

CREATE OR REPLACE PROCEDURE silver.load_tables(
	)
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE
...
BEGIN
	...
	BEGIN
		...
		SELECT
			...
			END AS sls_order_dt,
			CASE
				WHEN LENGTH(CAST(sls_ship_dt as VARCHAR)) != 8 THEN NULL
				ELSE CAST(CAST(sls_ship_dt as VARCHAR) as DATE)
			END AS sls_ship_dt,
			CASE
				WHEN LENGTH(CAST(sls_due_dt as VARCHAR)) != 8 THEN NULL
	...
	BEGIN
		...
		SELECT
			...
			CASE
				...
				WHEN UPPER(TRIM(cntry)) IN ('US', 'USA') THEN 'USA'
...
END;
$BODY$;
ALTER PROCEDURE silver.load_tables()
    OWNER TO postgres;
